...
/-- The state of a debate.  Either
    1. An uninterrupted List.Vector Bool n trace, or
...
def Alice' (o : OracleId) := (n : ℕ) → List.Vector Bool n → Comp {o} ℝ
...
/-- Bob sees the transcript and Alice's claimed probability, and decides whether to continue.
    ...
    mimic Oracle.fold. -/
def Bob' (o : OracleId) := (n : ℕ) → List.Vector Bool n → ℝ → Comp {o} Bool

/-- Bob using the normal `BobId` -/
def Bob := Bob' BobId

/-- Verifiers have an identical type signature to Bob, but use weaker parameters. -/
def Vera := (n : ℕ) → List.Vector Bool n → ℝ → Comp {VeraId} Bool

...
def render_status(registry: dict, search_evidence: dict) -> str:
    ...
    lines = [
        ...
        "| Metric | Current |",
        "|---|---:|",
        f"| Survey results inventoried (seed map) | {len(results)} / {registry['survey']['expected_result_count']} |",
...
def render_state_snapshot(registry: dict, landscape: dict) -> str:
    ...
    lines = [
        ...
        f"- Workbench: **{len(artifacts)}** Lean declarations on **{lean_rows}** of",
        f"  **{len(results)}** survey rows; statement-match **{covered}/{len(results)}**;",
        f"  `RELATED`-only **{related_only}**; reviewed bridges **{reviewed}**;",
        f"  WRAPPER **{wrappers}** / BRIDGE **{bridges}**; candidates **{candidates}**;",
        f"  landscape **{len(landscape.get('entries', []))}** (`landscape.yaml`).",
        STATE_END,
    ]
    return "\n".join(lines)
...
def render_readme(readme: str, registry: dict) -> str:
    ...
    # Workbench-first scorecard; grades explained in methodology.
    generated = "\n".join(
        [
            README_START,
            "| Metric | Current |",
            "|---|---:|",
            f"| Survey rows with atlas Lean | "
            f"**{lean_rows} / {len(results)}** |",
            f"| … statement-match (`EXACT` / `EQUIVALENT`) | "
            f"**{covered} / {len(results)}** |",
            f"| … workbench formalizations (`RELATED` only) | "
            f"**{related_only} / {len(results)}** |",

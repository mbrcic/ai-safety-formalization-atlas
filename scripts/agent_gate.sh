...
# The declaration index is the elaborated environment this gate resolves names
# against: `validate_registry`, `validate_intake`, `check_statement_freeze`,
# `check_docstring_identifiers`, `check_cited_declarations`, `report_consumers`,
# `report_predicate_duplicates` and `test_validators` all read it. It is
# regenerated by CI, which then asserts the committed copy is current -- so a
# stale index fails the push and not the local run. That is the wrong way round:
# every check below it was resolving against an environment 20 commits old, and
# reported green. It runs first here for the same reason CI has it at all.
echo "==> generate_declaration_index --check"
python3 scripts/generate_declaration_index.py --check

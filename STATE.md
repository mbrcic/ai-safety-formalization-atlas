...
Updated: 2026-07-29
Current phase: v0.3.1 is published; v0.4.0 is a single-commit PR candidate.
The candidate adds the A1–A3/B1–B3/B7 compositional, wireheading, and
preference workbench increment. Those additions are kernel-clean but remain
paper-scoped `RELATED` results or landscape entries. BY-012 and BY-033 bridges
remain `REVIEWED` (the robot formalization remains `RELATED`).

<!-- BEGIN GENERATED REGISTRY SNAPSHOT -->
<!-- Generated by scripts/generate_registry_views.py; do not edit by hand. -->
- Workbench: **41** Lean declarations on **13** of
...
- A1–A3/B1–B3/B7 workbench increment: operational hyperproperty topology and
  self-composition; finite-splice rectangularity and boundaries; explicit
  anonymous networks; finite Ring–Orseau equations; deterministic CRMDP
  complement construction; finite-percept goal-preservation induction step;
  source-parameterized preference-complexity and override results. Durable
  scope audit:
...
- v0.3.1 is published on `main`; v0.4.0 is prepared on
  `pr/workbench-increment` as one squash-ready commit (see
  [`docs/releases/v0.4.md`](docs/releases/v0.4.md)).
- Workbench increment (Compositional / Wireheading / Preference primary
  surfaces, consumers, registry/landscape wiring, contributor multi-track docs)
  is integrated locally as the v0.4.0 PR candidate. Facade tables are
  root-import compile contracts; no coverage relationship or AI bridge was
  graduated in this increment.
- Multi-commit local history remains on `agent-work` (and optional archive
  branches); public PR history is the squash commit only.
## Blocked

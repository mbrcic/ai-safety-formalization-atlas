...
release and nothing here supersedes it. What is on `control-and-computation` and
not in any release: Ashby's chapter 11 and Touchette–Lloyd's control limits at
printed scope behind an `AISafetyAtlas.Control` facade, the sharp
no-free-lunch characterization with the count that says almost no prior meets its
condition, Fano and data processing, and two joins that spend that material
rather than shelve it — `Knowledge.Entropy` puts Fano's floor on the knowability
kernel, and `Oversight.VarietyBound` separates what an overseer can see from what
it can do. `atlas-check` gains a `variety` kind whose false verdict is proved not
to be a clearance. A statement-by-statement audit of the five sources sits in
...
- Atlas Lean declarations: **208** (claim-row WRAPPER **13** / BRIDGE **5**).
- Results stating a source claim: **49**; recording a formalization only: **32** (**23** on the public root import).
...
- Rows carrying atlas Lean: **43** (**20** of them claim rows); catalogued candidate leads: **5**.

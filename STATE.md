...
`parsimony`: one results ledger instead of two, reporting absolute workbench
capital rather than survey-row fractions, splitting catalogued sources into
directories and works, indexing the atlas by mathematical area, opening a
conjecture ledger, and generating the contributor task board from `tasks.yaml`.
One row was retired (`LAND-MELO-001`, a paper recorded as a Lean formalization
and a second owner for BY-012's theorem) and two declarations were retyped from
`NEW_PROOF` to `WRAPPER`. No theorem statement, statement-match grade, or bridge
status changed on that branch.

<!-- BEGIN GENERATED REGISTRY SNAPSHOT -->
<!-- Generated by scripts/generate_registry_views.py; do not edit by hand. -->
- Atlas Lean declarations: **49** (claim-row WRAPPER **12** / BRIDGE **4**).

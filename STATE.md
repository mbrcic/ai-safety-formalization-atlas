...
Updated: 2026-08-08
Current phase: v0.5.0 is tagged and published. This release makes the
one-ledger, source-neutral workbench model explicit: absolute workbench metrics
replace survey-row fractions, catalogued sources have directory/work roles,
mathematical-area navigation is generated, conjectures have a contained intake
path, and the contributor task board is maintained in `tasks.yaml`. Validation
checks the process boundaries as well as the ledger shapes. No theorem
statement, statement-match grade, or bridge status changed in this release.
...
<!-- Generated by scripts/generate_registry_views.py; do not edit by hand. -->
- Package version: **`0.5.0`** (`lakefile.toml`, cross-checked against `CITATION.cff`).
- Latest recorded release note: [`docs/releases/v0.5.md`](docs/releases/v0.5.md).
...
- **Releases and what each shipped** — [`docs/releases/`](docs/releases/):
  ...
  [v0.3](docs/releases/v0.3.md), [v0.4](docs/releases/v0.4.md), and
  [v0.5](docs/releases/v0.5.md) (compositional / wireheading / preference
  increment, then the workbench-model and process release).

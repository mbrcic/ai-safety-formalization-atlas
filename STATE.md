...
- Atlas Lean declarations: **124** (claim-row WRAPPER **13** / BRIDGE **4**).
- Results stating a source claim: **49**; recording a formalization only: **28** (**19** on the public root import).
- Reviewed AI-system bridges: **2**; statement-reviewed only: **1**.
- Open conjectures: **1** of **1** recorded.
- Claim results with statement-match (`EXACT`/`EQUIVALENT`): **12**; with `RELATED`-only formalization: **7**. Counts are claim rows, not records: an artifact row's grade is on the row and never in this number.
- Rows carrying atlas Lean: **37** (**18** of them claim rows); catalogued candidate leads: **5**.

# The frontier hypotheses, and everything that stands on them
...
**Nothing supplies it, which is why the debt is availability and not effort.**
`NC-011` in [`docs/provenance/formalization-search.json`](formalization-search.json)
searched all six baseline corpora at their pinned revisions for the result and
for the apparatus a proof would need. Mathlib at the revision this repository
pins, the Isabelle AFP release `2026-02-06` — identified by recomputing the
archive digest the corpora record already carried — the Rocq undecidability
library, HOL4, HOL Light and the Agda standard library have, between them, no
Wishart density, no eigenvalue-decomposition Jacobian, no coarea formula, and no
Haar measure on the orthogonal group that a change of variables could run over.
The single recorded candidate is a false positive and is annotated as one: HOL
Light's `Multivariate/gamma.ml` matched on its path, and is the single-variable
Gamma function rather than the `Γ_p` that normalises the Wishart density.

The one signal worth watching is outside the corpora and is recorded in that
check's scope limits rather than as evidence: Tau Ceti's roadmap states
`nonsingularWishartMeasure` and `choleskyJacobianDensity` as targets, every one
of them `sorry`, and reaches the density on symmetric matrices by the Cholesky
route without stating the eigenvalue Jacobian at all — so even the one project
that has written this down as a goal has not written down the harder half.
...
`AISafetyAtlas.Conjectures.MAIS.A7ZetaVolumeBridge`, stated in
`AISafetyAtlas/Conjectures/MAIS/A7Zeta.lean`, frozen as
`AISafetyAtlas.Conjectures.MAIS.a7ZetaVolumeBridge_iff`.

### What is assumed

`MAIS-A7.tex` `def:llc` defines the local pair `(λ, m)` by the zeta integral

    Z(z; w*, δ) = ∫_{B_δ(w*)} |L(w) − L(w*)|^z dw,

meromorphically continued by resolution of singularities: `−λ` is its largest
pole and `m` that pole's order. The band-volume statement follows the
definition, introduced with the words *"In words:"* — a gloss, outside the
definition environment. Every MAIS-O7 and MAIS-O77 theorem in this repository
is about the gloss. This proposition is the substitution between the two, at the
germs A7 uses it on, and it is **not proved anywhere in the atlas**.

*Direction.* As with `O70-ZETA-BRIDGE`: print asserts an equivalence, the atlas
assumes only volume ⟹ zeta. That is the direction the O7 refutation and the O77
rows need, and the weaker, safer thing for a hypothesis to be. Neither frontier
is an equivalence, and prose that calls one an equivalence is describing print's
claim rather than the assumed proposition.

### Why it is not `O70-ZETA-BRIDGE`

Neither half of that frontier reaches A7.

Its germ is `rrrLossCoords M N H C` itself, because MAIS-O70's points are exact
factorizations where the loss vanishes and the centred band germ is the loss.
A7 quantifies over an arbitrary point of the loss, and print integrates the
centred band `|L − L(w*)|` there. At an exact factorization — MAIS-O7's terminal
rung, MAIS-O77(a)'s fiber — the centring collapses; at a nonterminal rung, which
is all of MAIS-O77(b), `L(w*) ≠ 0` and nothing about the loss alone constrains
that germ.

Its antecedent is `HasExactLocalPair`, a ratio tending to `1`. MAIS-O7 and
MAIS-O77(b) establish `HasLocalVolumeOrder`, two-sided bounds only. Assuming
`O70-ZETA-BRIDGE` would therefore not deliver the A7 rows even at the right
germ, because they do not supply its hypothesis. This one is stated over what is
actually proved, which makes it a strictly stronger assumption and says so.
...
the real-analyticity print carries, and the reduced-rank loss is polynomial in
its coordinates. Stating the bridge at the germ family it is consumed on keeps
the frontier at something that could be true.

`0 < lam` encodes print's own exclusion — `def:llc` requires `L` not identically
`L(w*)` near `w*`, "otherwise `Z` vanishes identically and the invariants are
undefined". A locally constant germ satisfies `HasLocalVolumeOrder` only in its
degenerate branch, where `lam = 0`.

### What stands on it

`o7_zeta_pair_rankZero`, `o7_zeta_pair_terminal` and `o7_zeta_refutation` give
MAIS-O7's two rung coefficients, and the failure of the printed increase,
in A7's own definition. `o77_saddle_zeta_pair` and
`o77_all_saddles_zeta_pair_one` do the same for MAIS-O77(b) at every point of
every nonterminal critical set.

None of these changes what is unconditionally true. The volume-order results
stand on their own and are what the ledger rows are graded on; these say what
follows for the zeta reading if the bridge holds.

### What does not stand on it

`pos_dims_of_rung` is unconditional: a nonterminal rung forces both ambient
dimensions positive, because `k < r` makes the target rank positive and a rank
bounds both sides of its shape. That is why the zeta consumers discharge the
bridge's non-degeneracy hypotheses rather than adding them.

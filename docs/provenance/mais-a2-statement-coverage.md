...
| O32 | `CONJ-022` *blocked* | `NotFormalized` | **Definition-blocked, and most of the vocabulary now exists** | The goal layer landed 2026-08-30 for MAIS-O33 and is shared: `Causal.ControlledProcess`, `Causal.Goal`, `Causal.GoalDynamics` and the first-action map `Causal.firstActionMapFull`. What is still absent is this row's own resolution radius over that family — the quantity `eps*(E,n,delta)` whose `1/n` asymptotics print asks about | This is the RABE25 goal setting, not the interventional causal model. Unlike O33, this row's question is about environments whose transitions read the action, so it would need the `ProbabilityTheory.Kernel.traj` congruence lemma O33's action-independent instance avoids |

...
- Four source targets have complete truth-valued propositions, all four at
  `Same` scope: O23, O25, O26, and the issue-#8 candidate answer to O31. O26
...
- Two source problems are covered in part: O29, whose part (a) is resolved, whose
  ...
  the negative margin subquestion and issue #4's complete singleton criterion --
...
- Of the seven registered A2 propositions, three are open (O25, O26, O31) and
  ...
  margin-sufficiency, O29(a)) and one affirmatively (issue #4's O34(a)
...
* **The exclusion exponents are uniform in `u` by print.** `prob:effective` says
  *"Find constants `a, b > 0` depending only on `(m, S)`"*. `O24Constants.a` and
  ...
  is a faithful transcription, which is what makes MAIS issue #7 an argument
...
choice of `(L, δmax)` remains between the two layers. No `O24Solution` is
exhibited, so non-vacuity remains an independent open debt.
...
**Consistency with the candidate on the tracker.** MAIS issue #8 claims, for the
...
part of #8's same-side prediction for two-node chains with the root intervened;
the two transition coordinates remain open. The box result adds a derived
quantity #8 does not address and contradicts nothing in #8.

**The induced thresholds are contained in a proper subinterval of the open unit
interval.** Issue #8's setup says *"let `t` in `(0,1)` be the utility decision

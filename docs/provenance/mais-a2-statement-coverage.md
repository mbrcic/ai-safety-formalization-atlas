...
**Every one of the fourteen targets is covered here**, and the `Ledger` column
says how. A *determine* problem has no truth-valued claim that is the same
...
of kind `target` names the specification a proposed answer must satisfy. Counting
only the rows that hold a `Prop` therefore undercounts what is covered.

**Six targets have no ledger row at all, by design.** MAIS-O2, O28, O29(c), O30,
O32 and O35 cannot be stated in this tree, and this matrix is where that is
recorded — with what is missing, at declaration level. Until 2026-08-30 each also
held a `blocked` row in `conjectures.yaml`; those rows carried no `lean`, no
`Prop` and no `refutation`, and minting a `CONJ-` number per unstatable problem
is what turns a selective conjecture board into a coverage index for one agenda.
They are retired verbatim in
[`retired-conjecture-rows.md`](retired-conjecture-rows.md) and summarised in the
`mais-open-problems-2026` entry of `registry.yaml`, the same place MAIS-O1 and
MAIS-O16 have always been recorded. Nothing about their statability changed.

## Matrix

| ID | Ledger | Scope | Lean statement status | What remains | Source-scope boundary |
|---|---|---|---|---|---|
| O2 | no ledger row — **recorded here** | — | **Definition-blocked** | Sampled-action transcript laws and independent response corruption at a known level | Two of the four things this row used to list arrived with `AISafetyAtlas.Causal.Query`: the analyst is randomized with perfect recall, and the risk is the expected error under a supremum over models and adversaries. What is missing is the *oracle*. `prob:noisy` replaces the policy-probability oracle by sampled actions, so a transcript's answers become draws rather than numbers, and the query layer observes exact real policy probabilities. **A sampled-answer transcript now exists for one channel**: `AISafetyAtlas.Conjectures.MAIS.O29Experiment` runs the same protocol with the answer drawn from the model's Boltzmann response. That is a precedent for the shape, not this row's oracle — `prob:noisy`'s answer is a sampled *action* under a supplied policy, not a binary Boltzmann response, and the O29 layer is written at `Bool` rather than at an arbitrary action type. Corruption is a second, independent gap |
...
| O32 | no ledger row — **recorded here** | — | **Definition-blocked, and most of the vocabulary now exists** | The goal layer landed 2026-08-30 for MAIS-O33 and is shared: `Causal.ControlledProcess`, `Causal.Goal`, `Causal.GoalDynamics` and the first-action map `Causal.firstActionMapFull`. What is still absent is this row's own resolution radius over that family — the quantity `eps*(E,n,delta)` whose `1/n` asymptotics print asks about | This is the RABE25 goal setting, not the interventional causal model. Unlike O33, this row's question is about environments whose transitions read the action, so it would need the `ProbabilityTheory.Kernel.traj` congruence lemma O33's action-independent instance avoids |
| O33 | `CONJ-023` *answer* | `Same` | **Resolved 2026-08-30** — `not_uniformlyTolerable` proves no positive `eta` is tolerable and `not_maisO33_etaStarPos` answers print's yes/no clause, both outright; the submitted value `eta* = 0` is graded by `Examples.Conjectures.MAIS.maisO33_etaStarIsZeroGivenBaseline_holds`, conditional on print's own cited baseline, since the unconditional `maisO33_etaStarIsZero_holds` can take its `0 <= eta*` half from the empty-supremum convention. The candidate is MAIS [issue #9](https://github.com/lionellevine/MAIS/issues/9)'s `eta* = 0` | Nothing for this row. The construction is `Examples.Causal.O33Corruption.exists_not_tolerantAt`; the layer it runs on is `Causal.Goal`, `Causal.ControlledProcess`, `Causal.GoalDynamics` and `Causal.Corruption` | **`Same` as `prob:corruption`.** Every printed quantifier is carried, including the two that make tolerability *uniform*: one randomized adaptive algorithm and one polynomial budget, across all finite instances with `|A| >= 2`, `n > 1` and `(n-1)(1-delta) > 4`. The adversary keeps print's power — `rho` is quantified after the strategy and is a function fixed before the interaction, which is what *persistent* means — and the witness uses less than that. The history axis is closed by proof rather than by prose: `Causal.inducedPolicy` computes the action a trajectory-prefix policy takes as a function of the states, and `Causal.optimalProbFull_eq` shows print's `max_{pi'}` over trajectory prefixes is this layer's supremum, so `isDeltaBoundedFull_lift_iff` makes the two boundedness clauses select the same agents. Three reading choices remain and `CONJ-023` records each: `max` is read as `sup`, which selects a sub-collection of `A(E,n,delta)` and so makes the refutation the stronger claim; the empty disjunction is excluded from `Psi_n`, which print's own depth clause settles since the empty set has no maximum; and the corruption map is total, which nothing observes because queries range over `S x Psi_n`. The goal event is measurable (`Causal.measurableSet_compositeSatisfies`), so `achieveProb` is print's probability and not only its canonical extension. The word *algorithm* is read from `subsec:queries`, which defines the analyst as one that *adaptively issues queries* and the figure of merit as an infimum over randomized analyst *strategies*, with `N` bounding queries rather than steps and no computational model stated anywhere in A2 — so `UniformAnalyst` is print's object. On the stricter reading in which *algorithm* means *computable*, the Lean would be wider, and that reading is refuted too: `Examples.Conjectures.MAIS.not_uniformlyTolerableWithin` refutes tolerability inside an arbitrary subclass of analysts. The determine-clause is certified twice — `maisO33_etaStarIsZeroGivenBaseline_holds` from print's own cited baseline with no empty-supremum convention, and `maisO33_etaStarIsZero_holds` unconditionally for this layer's totalized `etaStar`. Independent channel corruption from O2/O35 is a different model and is still absent |
...
| O35 | no ledger row — **recorded here** | — | **Definition-blocked** | The O34 two-variable real chart plus sampled-action minimax risk, regret adversaries, switching surfaces, and binary-channel capacity | Exact-query definitions cannot state the sampling/regret/corruption crossover |
...
- Five targets remain definition-blocked: O2, O28, O30, O32, O35.
- Of the eight registered A2 propositions, two are open (O25, O31) and six are
  resolved — four by checked negative witnesses (O23, O34(a) margin-sufficiency,
  O29(a), O33), one affirmatively (issue [#4](https://github.com/lionellevine/MAIS/issues/4)'s O34(a)
  singleton criterion), and one **vacuously** (O26, whose universal quantifier
  ranges over the empty set of O24 solutions and which therefore says nothing
  about the rate it is about). The defective O27 encoding is retained only as
  history, outside the ledger.

The dominant remaining dependencies are two independent foundations:

1. a sampled statistical-experiment layer (O2, O29(b)--(c), O35);
2. a resolution radius over the controlled-Markov/composite-goal layer (O32).
   The layer itself landed on 2026-08-30 with MAIS-O33; what O32 still needs is
   its own quantity `eps*(E,n,delta)`, and the trajectory-law congruence lemma
   that O33's action-independent instance was chosen to avoid.

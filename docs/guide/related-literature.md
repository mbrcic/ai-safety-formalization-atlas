...
| Breuer, *The Impossibility of Accurate State Self-Measurements*, Phil. Sci. 62(2) 197–214 (1995) | `breuer-1995-self-measurement` | Whether an apparatus embedded in a system can exactly measure every state of that system, when distinct global states share an apparatus reading | related-formal | `Knowledge.Embedded.no_meshing_inference_measures_all_states` (abstract Props 1–2, both proof routes); bridges in `.Composition` / `.Finite` | no (landscape `LAND-SELFMEAS-002/003`) | **Not** Rice/diagonal — the obstruction is indistinguishability, not self-reference. Grade **RELATED**: abstract core only; physical apparatus, dynamics, quantum and EPR omitted. Proper inclusion is **model data**, not derived from containment — the paper's own footnote 4 gives a contained apparatus without it. Source map [`../provenance/self-measurement-kernel.md`](../provenance/self-measurement-kernel.md) |
| Chandy & Lamport, *Distributed Snapshots* (TOPLAS 1985) | `chandy-lamport-1985` | Whether a process can determine a consistent global state while the computation continues | related-formal | none — Isabelle/AFP `Chandy_Lamport`, Path A, no Lean surface | no (landscape `LAND-CL-001`) | Reproduced, **ungraded**: the row records no `EXACT`/`EQUIVALENT` relationship to the paper, so do not read it as paper-fidelity coverage. **Possibility** result, catalogued because it bounds how a self-knowledge impossibility may be worded: snapshots escape by relaxing contemporaneity, so "a system cannot know its own global state" is false at useful resolutions. Boundary partner of `LAND-SELFMEAS-001` and `LAND-TEMPORAL-001` in [`../status/relations.md`](../status/relations.md) |
| Wang, Dorchen & Jin, [*On The Statistical Limits of Self-Improving Agents*](https://arxiv.org/abs/2510.04399) (2026) | `atlas-ref-wang-dorchen-jin-2026` | Whether policy-reachable representational self-modifications preserve a uniform distribution-free PAC guarantee | none | none — a repaired statement would require a future PAC/VC layer | no (related lit only) | **Audited proposal, not an endorsed frontier theorem.** Theorem 1's displayed `sup₍H₎ VC(H) < ∞` condition does not control adaptive selection among arbitrarily many classes. Its sufficiency proof adds a fixed common envelope `G_K` of finite VC dimension, an indexing-control assumption not supplied by A1–A7 or the displayed condition. Theorem 2 also derives an oracle bound against `inf₍G_K₎ R` from ERM only inside `H_T`; the valid comparator is `inf₍H_T₎ R` unless an approximation or optimization premise is added. Useful self-modification framing; defer formalization until repaired. |
| Majumdar, [*The Relativity of AGI*](https://arxiv.org/abs/2601.17335) (2026) | `atlas-ref-majumdar-2026` | Whether a distribution-relative AGI predicate has a sound-and-complete computable certifier over all admissible agents | related-formal | `Verification.rice` identifies the intended classical mechanism, conditionally | no (related lit only) | **Related mechanism, not a reproduced application.** Semanticity and nontriviality on an arbitrary effectively presented subclass of admissible agents do not by themselves license Rice's theorem; the paper needs an acceptable numbering, universality, or an explicit property-preserving reduction. Corollary 7.1 merely externalizes the claimed universal internal procedure and adds no BY-044-style self-reference. Proposition 7.1's proof also searches for proofs of negations without assuming the required negative-instance completeness, so no Tarski hook is claimed. |
